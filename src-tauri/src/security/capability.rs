...
pub const MAX_ISSUED_AGE_MS: i64 = 30_000;
...
pub fn verify_capability_token(
    ...
    let token: CapabilityToken = serde_json::from_str(token_json)
        .map_err(|_| "capability_token_invalid")?;
    let claims = &token.claims;
    if claims.version != 1 { return Err("capability_version_invalid"); }
    ...
    if claims.operation != expected_operation { return Err("capability_operation_denied"); }
    if claims.risk != expected_risk { return Err("capability_risk_mismatch"); }
    if claims.expires_at <= now || claims.issued_at > now + MAX_CLOCK_SKEW_MS {
        return Err("capability_expired");
    }
    if claims.expires_at <= claims.issued_at || claims.expires_at - claims.issued_at > MAX_CAPABILITY_TTL_MS {
        return Err("capability_ttl_invalid");
    }
    if claims.nonce != expected_nonce { return Err("capability_nonce_mismatch"); }

    let key_bytes = URL_SAFE_NO_PAD.decode(issuer_public_key_b64)
        .map_err(|_| "capability_issuer_key_invalid")?;
    let key_array: [u8; 32] = key_bytes.try_into()
        .map_err(|_| "capability_issuer_key_invalid")?;
    let key = VerifyingKey::from_bytes(&key_array)
        .map_err(|_| "capability_issuer_key_invalid")?;
    let sig_bytes = URL_SAFE_NO_PAD.decode(&token.signature)
        .map_err(|_| "capability_signature_invalid")?;
    let sig = Signature::from_slice(&sig_bytes)
        .map_err(|_| "capability_signature_invalid")?;
    let payload = serde_json::to_vec(claims)
        .map_err(|_| "capability_payload_invalid")?;
...
mod tests {
    ...
    fn accepts_valid_signed_token() {
        ...
        let got = verify_capability_token(&token, &claims.capability_id, &claims.session_id, &claims.operation, &claims.risk, &claims.nonce, 1_005_000, &public).unwrap();
    ...
    fn rejects_tampered_claims() {
        ...
        let public = URL_SAFE_NO_PAD.encode(key.verifying_key().to_bytes());
        assert!(verify_capability_token(&token, &claims.capability_id, &claims.session_id, "sandbox.write_text", &claims.risk, &claims.nonce, 1_005_000, &public).is_err());
    }

    #[test]
    fn rejects_long_ttl() {
        let (token, key, mut claims) = fixture();
        claims.expires_at = claims.issued_at + MAX_CAPABILITY_TTL_MS + 1;
        let payload = serde_json::to_vec(&claims).unwrap();
        let sig = key.sign(&payload);
        let token = serde_json::json!({"claims": claims, "signature": URL_SAFE_NO_PAD.encode(sig.to_bytes())});
        let public = URL_SAFE_NO_PAD.encode(key.verifying_key().to_bytes());
        assert_eq!(verify_capability_token(&token.to_string(), &claims.capability_id, &claims.session_id, &claims.operation, &claims.risk, &claims.nonce, 1_005_000, &public), Err("capability_ttl_invalid"));
    }
}
