...
describe('native host boundary', () => {
  it('allows only the fixed declarative operation vocabulary', async () => {
    const calls: unknown[] = [];
    const host = new NativeHostBoundary({ ...opts, sessionId, invoke: { invoke: async (_command, payload) => { calls.push(payload); return { ok: true }; } } });
  ...
  it('rejects shell-like or unknown operations before native IPC', async () => {
    const host = new NativeHostBoundary({ ...opts, sessionId, invoke: { invoke: async () => ({ ok: true }) } });
  ...
  it('bounds host content before IPC', async () => {
    const host = new NativeHostBoundary({ ...opts, sessionId, invoke: { invoke: async () => ({ ok: true }) }, policy: { maxContentBytes: 4 } });
  ...
  it('forces sandboxOnly even if caller tries to override policy', () => {
    expect(() => new NativeHostBoundary({ ...opts, sessionId, invoke: { invoke: async () => ({}) }, policy: { sandboxOnly: false as true } })).not.toThrow();
  });
});
